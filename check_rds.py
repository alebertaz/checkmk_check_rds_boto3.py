...
class RDS(object):
    ...
    def __init__(self, region, profile=None, identifier=None):
        ...
        if self.region == 'all':
            ...
            self.regions_list = [region['RegionName'] for region in ec2.describe_regions()["Regions"]]
        else:
        ...
        if self.identifier:
            for reg in self.regions_list:
                try:
                    rds = boto3.client('rds', region_name=reg, config=my_config)
    ...
    def get_metric(self, metric, start_time, end_time, step):
        ...
        cw_conn = boto3.client('cloudwatch', region_name=self.region, config=my_config)
        ...
        if result['Datapoints']:
            if len(result['Datapoints']) > 1:
                # Get the last point
                result['Datapoints'] = sorted(result['Datapoints'], key=lambda k: k['Timestamp'])
                result['Datapoints'].reverse()

            result = float('%.2f' % result['Datapoints'][0]['Average'])
